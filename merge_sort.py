...
def merge_sort_bottom_up(list_to_be_sorted: [int]) -> [int]:
...
def merge_sort_recursion(list_to_be_sorted: [int], visualize: bool = False) ->[int]:
    ...
    if len(list_to_be_sorted) == 1:
        ...
        if visualize:
            print("recurse on the left list")
        left_list_sorted = merge_sort_recursion(left_list, visualize)
        if visualize:
            print("recurse on the right list")
        ...
        if visualize:
            print("list {} is break down to".format(list_to_be_sorted))
            print("{} and {}".format(left_list, right_list))
            print("sorted to {} and {}".format(left_list_sorted, right_list_sorted))
            print("combined and sorted to {}".format(result_after_sort))
...
if __name__ == "__main__":
    ...
    analyze("Jieshu's merge Sort", merge_sort_recursion, random_list, visualization_on)
